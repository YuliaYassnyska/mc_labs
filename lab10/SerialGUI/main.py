        ...
        if serial_port.isOpen():
            ...
            if data:
                ...
                if received.endswith("*"):
                    # show_data(selected_slave, 'Res: ')
                    # show_data(selected_slave, received)
                    # show_data(selected_slave, '\n')
                    received = received[1:-1].split(';')
                    crc = int('0x' + received[0], 16)
                    check_ok = check_crc(received[1], crc, CRC_BIT_LENGTH, POLY, INIT_CRC, REF_IN, REF_OUT, XOR_OUT)
                    sensor_data = received[1].split(',')
                    show_data(selected_slave, 'Humidity: ' + str(sensor_data[0]))
                    show_data(selected_slave, '\n')
                    show_data(selected_slave, 'Temperature: ' + str(sensor_data[1]))
                    show_data(selected_slave, '\n')
                    show_data(selected_slave, '-------------------\n')
                    received = ''
